...
public class RequestHandler implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        if (session != null) {
            ...
            if (loggedIn) {
                ...
                if (!isResource) {
                    ...
                    } else if (request.getRequestURI().contains("rating")) {
                        historyItem.setTitle("Bewertung");
                    }
        ...
        /* Exceptions */
        if (isStartPage || isLoginRequest || isRegistrationRequest || isResource || request.getRequestURI().startsWith(request.getContextPath() + ResourceHandler.RESOURCE_IDENTIFIER)) {
            filterChain.doFilter(request, response);
        } else {
